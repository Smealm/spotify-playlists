...
# The archive repository/location can be changed here once
# if the hosting repository or URL structure ever changes.
#
# {playlist_id} is replaced with --archive-playlist-id.
#
# Individual GitHub Actions / scheduled jobs therefore only
# need to know the Spotify source playlist ID.
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Spotify playlist ID used to locate the cumulative "
            "Spotify Playlist Archive Markdown file."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Project-relative path for the run summary log. "
            "Example: /logs/creamy-output.txt"
        ),
    )
...
def resolve_log_path(log_path):
    """
    Resolve --log as a path relative to the project directory.

    Examples:

        /logs/creamy-output.txt
        logs/creamy-output.txt

    Both resolve to:

        <project>/logs/creamy-output.txt

    Leading slash/backslash characters are intentionally
    stripped so the argument always represents a project-
    relative path rather than an operating-system absolute path.
    """
    ...
    # Prevent --log from escaping the project directory.
    try:
...
class RunLogger:
    """
    Stores only the concise run summary.

    The normal console output remains verbose, but only the
    summary information is written to the log file.

    The log file is replaced on every run.
    """

    def __init__(self, log_path):
        self.path = resolve_log_path(log_path)
        self.lines = []
    ...
    def write(self):
        ...
        print(
            f"Run summary written to: {self.path}"
        )
...
class TrackMetadataCache:
    """
    Run-wide Spotify metadata cache.

    A Spotify track ID is fetched from the API at most once
    during this script invocation.
    ...
    def fetch(
        ...
        """
        Fetch metadata only for IDs that are not already
        present in the run-wide cache.
        """
        ...
        print(
            ...
            f"{len(missing_ids)} new tracks "
            f"({len(unique_ids) - len(missing_ids)} "
            f"already cached)..."
        )
...
def add_tracks(
    ...
    Returns statistics about the actual additions and any
    candidates skipped by the final live duplicate check.
    """

    stats = {
        "candidates": len(tracks_to_add),
        "added": 0,
        "final_exact_duplicates": 0,
        "final_fuzzy_duplicates": 0,
    }
    ...
    for start in range(
        ...
        current_metadata = metadata_cache.fetch(
            current_ids,
            access_token,
        )
...
def reorder_playlist(
    ...
    for target_position in range(
        ...
        # Detailed move information is intentionally printed
        # to the console only and is NOT included in the log.
        print(
            f"Move #{moves + 1}: "
    ...
    print(
        ...
        f"{moves} move(s) "
        f"in {reorder_duration:.2f} seconds."
    )
...
def write_summary(
    ...
    """
    Write the concise run summary.

    The existing log file is replaced rather than appended.
    """
    ...
    LOGGER.add(
        f"Archive tracks detected: "
        f"{len(archive_tracks)}"
    )

    LOGGER.add(
        f"Exact duplicates skipped: "
        f"{dedup_stats['exact_duplicates']}"
    )

    LOGGER.add(
        f"Fuzzy duplicates skipped: "
        f"{dedup_stats['fuzzy_duplicates']}"
    )

    LOGGER.add(
        f"Missing metadata skipped: "
        f"{dedup_stats['missing_metadata']}"
    )

    LOGGER.add(
        f"Tracks initially identified as new: "
        f"{len(tracks_to_add)}"
    )

    LOGGER.add(
        f"Tracks actually added: "
        f"{add_stats['added']}"
    )

    final_skipped = (
        add_stats["final_exact_duplicates"]
        + add_stats["final_fuzzy_duplicates"]
    )

    LOGGER.add(
        f"Final duplicate check skipped: "
        f"{final_skipped}"
    )

    total_skipped = (
        dedup_stats["skipped_during_detection"]
        + final_skipped
    )

    LOGGER.add(
        f"Total skipped during run: "
        f"{total_skipped}"
    )
    ...
    LOGGER.add(
        f"Reorder moves:        "
        f"{reorder_stats['moves']}"
    )

    LOGGER.add(
        f"Reorder duration:     "
        f"{reorder_stats['duration_seconds']:.2f} seconds"
    )
...
def main():
    ...
    print(
        f"Archive playlist ID: {ARCHIVE_PLAYLIST_ID}"
    )
    ...
    # --------------------------------------------------------
    # 3. Create run-wide metadata cache.
    #
    # Every Spotify track ID fetched during this run is
    # remembered here and reused later.
    # --------------------------------------------------------

    metadata_cache = TrackMetadataCache()

    # --------------------------------------------------------
    # 4. Read destination playlist.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 5. Fetch existing playlist metadata.
    #
    # This populates the run-wide cache.
    # --------------------------------------------------------

    existing_metadata = metadata_cache.fetch(
        existing_tracks,
        access_token,
    )

    # --------------------------------------------------------
    # 6. Fetch archive metadata.
    #
    # Already-cached tracks are NOT requested again.
    # --------------------------------------------------------

    archive_metadata = metadata_cache.fetch(
        archive_tracks,
        access_token,
    )

    # --------------------------------------------------------
    # 7. Determine tracks that need to be added.
    # --------------------------------------------------------

    (
        tracks_to_add,
        dedup_stats,
    ) = determine_tracks_to_add(
        archive_tracks,
        archive_metadata,
        existing_metadata,
    )
    ...
    # --------------------------------------------------------
    # 8. Add genuinely new tracks.
    # --------------------------------------------------------

    add_stats = add_tracks(
        tracks_to_add,
        access_token,
        metadata_cache,
    )
    ...
    # --------------------------------------------------------
    # 9. Re-read playlist after additions.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 10. Fetch fresh metadata.
    #
    # The cache means existing/archive tracks already fetched
    # during this run are reused. Only genuinely unseen IDs
    # are sent to Spotify.
    # --------------------------------------------------------
    ...
    reorder_stats = reorder_playlist(
        current_tracks,
        desired_tracks,
        access_token,
    )
    ...
    # --------------------------------------------------------
    # 13. Write concise summary log.
    #
    # The log file is replaced every run.
    # --------------------------------------------------------
